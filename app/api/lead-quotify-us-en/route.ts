...
function validateUsPhone(value: unknown): PhoneValidationResult {
  ...
  if (normalized.length !== 10) {
    return {
      ...
      reason: "Enter a valid 10-digit U.S. phone number.",
    };
  }
  ...
  if (!/^[2-9]\d{2}[2-9]\d{6}$/.test(normalized)) {
    return {
      ...
      normalized,
      flags: ["invalid_nanp"],
      reason: "Enter a valid U.S. mobile or landline number.",
    };
  }
  ...
  if (flags.length > 0) {
    return {
      ...
      normalized,
      flags,
      reason: "Enter a real U.S. phone number. Avoid sequences or example numbers.",
    };
  }
...
export async function POST(request: Request) {
  ...
  const webhookUrl = process.env.QUOTIFY_US_EN_WBK?.trim();
  ...
  if (!phoneValidation.isValid) {
    return NextResponse.json(
      {
        error: phoneValidation.reason || "Enter a valid U.S. phone number.",
        riskFlags,
      },
      { status: 422 }
    );
