...
const disqualificationCookieName = "best_life_call3_disqualified";
...
const callFunnelPagePath = "/call-3";
...
function SolidCheckIcon({ className = "h-[1em] w-[1em]" }: { className?: string }) {
  return (
...
function ClockOutlineIcon({ className = "h-[1em] w-[1em]" }: { className?: string }) {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24" className={className} fill="none">
...
function DollarCircleIcon({ className = "h-[1em] w-[1em]" }: { className?: string }) {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24" className={className} fill="none">
      <circle cx="12" cy="12" r="9" stroke="currentColor" strokeWidth="2" />
      <path d="M15.5 8.5h-4.25a1.75 1.75 0 0 0 0 3.5h1.5a1.75 1.75 0 0 1 0 3.5H8.5M12 6.8v10.4" stroke="currentColor" strokeLinecap="round" strokeWidth="2" />
    </svg>
  );
}
...
function trackCallFunnelEvent(eventName: "ViewContent" | "Contact", currentStep: FunnelStep) {
  ...
  const eventParams = { page: "/call-3", step: currentStep };
...
function CallFunnelPixels({ currentStep }: { currentStep: FunnelStep }) {
  ...
  return (
    <>
      <Script id="meta-pixel-call-3" strategy="afterInteractive">
        {`
      ...
      <Script id="tiktok-pixel-call-3" strategy="afterInteractive">
        {`
          !function (w, d, t) {
...
export default function Home() {
  ...
  const visibleQuestionSteps = ["age", "goal", "call-page"] as FunnelStep[];
  ...
  function handleAgeChoice(option: string) {
    ...
    window.setTimeout(() => {
      transitionTo(option === "65+" ? "disqualified" : "goal", "forward");
    }, 120);
  }
  ...
  function renderQuestionnairePanel() {
    const ringbaTags = JSON.stringify({
      call3_age_group: answers.ageGroup,
      call3_insurance_goal: answers.insuranceGoal,
    });
    ...
    return (
      <div key={`panel-${panelKey}`} className="w-full">
        {currentStep === "call-page" ? (
          <>
            <Script id="ringba-call-3-tags" strategy="afterInteractive">
              {`
                window._rgba_tags = window._rgba_tags || [];
                window._rgba_tags.push(${ringbaTags});
              `}
            </Script>
            <Script id="ringba-call-3-number-pool" src={ringbaScriptUrl} strategy="afterInteractive" />
          </>
        ) : null}
          ...
          <div className="flex w-full items-center justify-between gap-3 md:gap-4">
            ...
            <div className="flex w-[58px] shrink-0 justify-end md:w-[70px]">
              <span className="whitespace-nowrap text-[12px] font-black tracking-[-0.02em] text-[var(--brand-dark)] md:text-[13px]">
                {currentQuestionIndex >= 0 ? `${currentQuestionIndex + 1} de 3` : "1 de 3"}
              </span>
            </div>
          </div>
            ...
            <div className={`mt-8 grid w-full max-w-[460px] gap-4 md:mt-10 ${animationClass}`}>
              {goalOptions.map((option) => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    handleDirectChoice("insuranceGoal", option, "call-page")
                  }
                  className={optionButtonClass(
          ...
          {currentStep === "call-page" ? (
            <div className={`mt-8 flex w-full max-w-[460px] flex-col items-center text-center md:mt-10 ${animationClass}`}>
              <div className="w-full rounded-[18px] border border-[#d8e6ff] bg-[#f7fbff] px-5 py-6 shadow-[0_10px_24px_rgba(26,115,232,0.08)]">
                <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[#18bf79] text-white">
                  <SolidCheckIcon className="h-6 w-6 text-white" />
                </div>
                ...
                <a
                  href={ringbaTelHref}
                  className="mt-2 block text-[28px] font-black tracking-[-0.04em] text-[#101820] transition hover:text-[var(--brand)]"
                >
                  {ringbaNumberToReplace}
                </a>
                <a
                  href={ringbaTelHref}
                  className="call-cta-shake mx-auto mt-4 inline-flex min-h-[46px] w-full max-w-[240px] items-center justify-center gap-2 rounded-full bg-[var(--brand)] px-5 py-3 text-white shadow-[0_10px_20px_rgba(26,115,232,0.2)] transition hover:bg-[var(--brand-dark)]"
                >
                  <CallOutlineIcon className="h-[19px] w-[19px] shrink-0 text-white" />
                  <span className="text-center text-[14px] font-extrabold leading-[1.15] tracking-[-0.02em] text-white md:text-[15px]">
                    LLAMAR AHORA
                  </span>
                </a>
                <p className="mx-auto mt-3 max-w-[320px] text-[14px] leading-[1.45] text-[#5d6674]">
                  Toca el número para llamar a un asesor y reclamar tu beneficio.
                </p>
                <div className="mx-auto mt-5 max-w-[360px] border-t border-[#dbe8ff] pt-5">
                  <p className="text-[18px] font-black leading-[1.2] tracking-[-0.04em] text-[#101820]">
                    Descubre cuánto podrías recibir con tu Seguro IUL.
                  </p>
                  <p className="mt-3 text-[14px] leading-[1.5] text-[#5d6674]">
                    Un agente licenciado te dará una estimación personalizada y verificará si calificas. Sin costo ni compromiso, todo por teléfono en minutos.
                  </p>

                  <div className="mt-5 grid gap-3 text-left">
                    <div className="flex items-center gap-3 rounded-[12px] bg-white px-4 py-3 shadow-[0_6px_16px_rgba(26,115,232,0.08)] ring-1 ring-[#e7efff]">
                      <ClockOutlineIcon className="h-5 w-5 shrink-0 text-[var(--brand)]" />
                      <span className="text-[14px] font-extrabold text-[#101820]">
                        Estimación en Minutos
                      </span>
                    </div>
                    <div className="flex items-center gap-3 rounded-[12px] bg-white px-4 py-3 shadow-[0_6px_16px_rgba(26,115,232,0.08)] ring-1 ring-[#e7efff]">
                      <StatisticGrowIcon className="h-5 w-5 shrink-0 text-[var(--brand)]" />
                      <span className="text-[14px] font-extrabold text-[#101820]">
                        Crecimiento Ligado al Mercado
                      </span>
                    </div>
                    <div className="flex items-center gap-3 rounded-[12px] bg-white px-4 py-3 shadow-[0_6px_16px_rgba(26,115,232,0.08)] ring-1 ring-[#e7efff]">
                      <DollarCircleIcon className="h-5 w-5 shrink-0 text-[var(--brand)]" />
                      <span className="text-[14px] font-extrabold text-[#101820]">
  ...
  return (
    <main className="min-h-screen bg-[var(--page-bg)] text-[var(--ink)]">
      ...
      <style jsx global>{`
        ...
        .call-cta-shake {
          animation: call_cta_shake 1.8s ease-in-out infinite;
      ...
      <footer className="px-4 pb-5 pt-3 text-center text-[9px] leading-[1.45] text-[#b8bec8] md:text-[10px]">
        ...
        {currentStep === "call-page" ? (
          <p className="mx-auto mt-2 max-w-[720px]">
