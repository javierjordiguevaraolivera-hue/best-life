...
export default function Home() {
  ...
  function renderQuestionnairePanel() {
    const ringbaTags = JSON.stringify({
      call4_age_group: answers.ageGroup,
      call4_insurance_goal: answers.insuranceGoal,
    });
    ...
    return (
      <div key={`panel-${panelKey}`} className="w-full">
        {currentStep === "call-page" ? (
          <>
            <Script id="ringba-call-4-tags" strategy="afterInteractive">
              {`
                window._rgba_tags = window._rgba_tags || [];
                window._rgba_tags.push(${ringbaTags});
              `}
            </Script>
            <Script id="ringba-call-4-number-pool" src={ringbaScriptUrl} strategy="afterInteractive" />
          </>
        ) : null}
