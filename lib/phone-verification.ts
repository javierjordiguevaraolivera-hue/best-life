...
export function createPhoneVerificationToken(normalizedPhone: string, now = Date.now()) {
...
export function validatePhoneVerificationToken(
  ...
  try {
    ...
    ) as Partial<PhoneVerificationTokenPayload>;
...
export async function verifyPhoneWithVeriphone(
  value: unknown,
  options: {
  ...
  try {
    ...
    return {
      ...
      veriphone: {
        normalized,
        phoneValid: true,
        phoneType: phoneType as PhoneVerificationEvidence["phoneType"],
        carrier,
        countryCode,
        country,
...
export function validateLeadPhoneVerification({
  phone,
  verificationToken,
  verification,
  now,
  ...
  const flags: string[] = [];
  const evidence = verification as Partial<PhoneVerificationEvidence> | null;
  ...
  return {
    ...
    normalized,
    flags,
    evidence: flags.length === 0 ? (evidence as PhoneVerificationEvidence) : null,
  };
}
